...
import impl.App;
import impl.Attack;
import impl.BattleBackground;
import impl.OOB;
import simulation.comparators.WeaponComparator;
...
public class Craft implements Serializable, Movable, Comparable<Craft>, RadarVehicle {
    ...
    /**
     * Method adds part to the craft battleGui.crafts.systems.
     *
     * @param system to add.
     */
     ...
     * Simulates adding fired weapons to lost battleGui.crafts.systems.
     *
    ...
    public boolean fire(TreeMap<Craft, LinkedList<Weapon>> target) {
        for (Weapon weaponSystem : target.firstEntry().getValue()) {
            ...
            if (weaponSystem instanceof Missile) {
                ...
                double angle = BattleBackground.hitAngle(this, target.firstKey());
            ...
            if (part instanceof Armor) {
                for (ArmorSide side : ArmorSide.values()) {
                    if (angle < side.getMaxAngle() && angle > side.getMinAngle()) {
                        if (((Armor) part).penetrateArmor(side, weaponSystem.getAmmunition().getPenetration(), angle)) {
     ...
     */
    public TreeMap<Craft, LinkedList<Weapon>> findAndEngage() {
        if (lastFired > 0) {
            return null;
        }
        ...
        //All weapons sorted by range and damage with range against the closest possible enemy.
